<template>
	<div class="debtlist">
		...
		<blank></blank>
		<div class="topfen">
			<p style="margin-left: 285px;border-bottom: 0;font-weight: 400;" @click="$router.push('/debt/debtList')">P2P</p>
			<p @click="$router.push('/debt/debtLista')">借条</p>
		</div>
		<!--模糊查询开始-->
		<div class="debtlist-nav">
			<div class="blank" @click="sortShow = false;"></div>
			<form @click="sortShow = false;"> 
				<input type="text" v-model="optionParams.loansId" name="" id="" placeholder="请输入订单号" value="" /> 
				<!--<input type="text" v-model.trim="favorites" v-on:input="getPlatform()" placeholder="请输入订单号" @blur="pfNameList = [];">-->
			...
			<div class="filter">
				...
				<div v-if="optionParams.amountSort === 0" style="margin-left:123px;" @click="optionParams.rateSort = 0;optionParams.amountSort = 2;getApplyList();">
					<span style="margin-right:17px;">成交金</span><img src="~@/assets/images/zhuangtailing.png" alt="" srcset="">
				</div>
				<div v-if="optionParams.amountSort === 2" style="margin-left:123px;color:rgba(0,0,0,1);" @click="optionParams.rateSort = 0;optionParams.amountSort = 1;getApplyList();">
					<span style="margin-right:17px;font-weight:bold;">成交金</span><img src="~@/assets/images/zhuangtaiyi.png" alt="" srcset="">
				</div>
				<div v-if="optionParams.amountSort === 1" style="margin-left:123px;color:rgba(0,0,0,1);" @click="optionParams.rateSort = 0;optionParams.amountSort = 0;getApplyList();">
					<span style="margin-right:17px;font-weight:bold;">成交金</span><img src="~@/assets/images/zhuangtaier.png" alt="" srcset="">
				</div>
				<div v-if="optionParams.rateSort === 0" style="margin-left:107px;" @click="optionParams.amountSort = 0;optionParams.rateSort = 2;getApplyList();">
					<span style="margin-right:17px;">折扣</span><img src="~@/assets/images/zhuangtailing.png" alt="" srcset="">
				</div>
				<div v-if="optionParams.rateSort === 2" style="margin-left:107px;color:rgba(0,0,0,1);" @click="optionParams.amountSort = 0;optionParams.rateSort = 1;getApplyList();">
					<span style="margin-right:17px;font-weight:bold;">折扣</span><img src="~@/assets/images/zhuangtaiyi.png" alt="" srcset="">
				</div>
				<div v-if="optionParams.rateSort === 1" style="margin-left:107px;color:rgba(0,0,0,1);" @click="optionParams.amountSort = 0;optionParams.rateSort = 0;getApplyList();">
		...
		<div class="filterDailog" v-if="filterDailogShow">
			...
			<div class="right">
				...
				<div class="itemList" style="margin-top: 90px;">
					...
					<div class="selectBox">
						<div class="item" v-for="(item,index) in statusLista" :key="index">
							<img v-if="!item.isShow" src="~@/assets/images/statusNone.png" alt="" @click="item.isShow = !item.isShow;">
							<img v-if="item.isShow" src="~@/assets/images/statusSelect.png" alt="" @click="item.isShow = !item.isShow;">
							<div class="name" style="line-height:46px;">{{item.name}}</div>
						</div>
...
export default {
	...
	data() {
		return {
			...
			statusLista: [
				{ name: "个人", isShow: false, status: 1 },
				{ name: "小贷", isShow: false, status: 2 },
			],
			typeList: [
...
//		},
		...
		confirmSeach() {
			...
			this.statusLista.forEach((item, index) => {
				if (item.isShow) {
					this.optionParams.type.push(item.status);
				}
			})
			this.getApplyList();
			this.filterDailogShow = false;
