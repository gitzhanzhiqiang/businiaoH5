<template>
	<div class="cash">
		...
		<div class="btm">
			<button class="pinz" v-if="rowData.orientFlag===1" @click="showPwd()">债权密码</button>
			<button class="yanc" :class="{active:rowData.orientFlag!=1}" @click="cancelTransfer()">撤销转让</button>
		</div>
		<messageBox :isShow="dailog.isShow" :message="dailog.message" :title="dailog.title"></messageBox>
		<alertBox :isShow="alertData.isShow" :message="alertData.message" :title="alertData.title"></alertBox>
	</div>
</template>
<script>
import ajax from '@utils/config';
import headCon from '@/components/header.vue';
import messageBox from '@/components/confirmBox.vue';
